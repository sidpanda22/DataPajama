...
from slimpajama.utils.utils import (
    rm_if_exists,
    write_lmd_dataset,
)
...
def pass_1_shuffle(
    ...
    ars = [
        ...
        lmd.Archive(f"{output_dir_path}/chunk{i}")          #changed for mac
        for i in range(n_process)
    ]

    # queue to collect documents from reading processes
    # docs_queue = [Queue(64 * 10000) for _ in range(n_process)]
    docs_queue = [Queue(2**15-1) for _ in range(n_process)]          #changed for mac
